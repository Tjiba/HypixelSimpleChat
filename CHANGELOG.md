...
- SkyBlock XP: `+2 SkyBlock XP (Bag Upgrades) (69/100)` is shortened to `+2 SB XP (Bag Upgrades)`. A guard rule keeps any XP gain from being swallowed by a generic rule.
- `/hsc debug`, which logs each message with its verdict, the rule that matched, and why a guard kept it as-is.
...
### Added
- Per-channel formatting for Public / Party / Guild / Officer, each with `Vanilla` or `Compact` mode, rank-colored names, toggleable Hypixel & guild ranks, custom prefixes and colors.
...
- Compact reformats for damage numbers, kill combos, mob abilities, sacks, loot share, NPC dialog, boss lines, boosters, daily/mystery rewards, rare rewards, GEXP, server routing, profile ID, and more.
- Custom config menu: glass UI (rounded corners, blurred background), sidebar navigation, live preview, tooltips and an HSV color picker. Same menu on both MC versions. Mod Menu integration.
- Chat tabs (All / Party / Guild) above the chat input. Route what you send to the selected channel, or filter the view to it.
- Ctrl+F search to live-filter the chat history from the chat input.
- Right-click to copy any chat line ("Copied!" feedback).
- Extended chat history, up to 2048 messages instead of vanilla's 100.
