...
class MenuScreen(private val parent: Screen?) : Screen(
    ...
    // bulkIds non nul = barre « tout mettre à » : le choix est recopié dans ces réglages-là.
    private class Setting(
        val entry: ConfigEntry, val id: String, val title: String, val kind: Kind, val desc: String,
    ...
    private val bulkEntries = HashMap<String, ConfigEntry>()
    // Groupes dépliés. Repliés par défaut : la page reste courte, le détail est à un clic.
    private val expanded = HashSet<String>()

    /** Barre d'un groupe : une liste déroulante qui recopie son choix dans toutes ses phrases. */
    private fun bulkRow(groupId: String): Setting? {
        val ids = MenuLayout.bulk[groupId] ?: return null
        val (title, desc) = MenuLayout.titles[groupId] ?: return null
        val entry = bulkEntries.getOrPut(groupId) {
            ConfigEntry(groupId, EntryKind.ENUM, title, desc, RuleAction.HIDE, RuleAction.entries.toList())
        }
        return Setting(entry, groupId, title, Kind.DROPDOWN, desc, ids)
    }

    /** Barre en haut de page : applique un choix à tous les réglages de règles de la catégorie. */
    private fun categoryRow(categoryId: String): Setting? {
        val ids = MenuLayout.categoryBulk[categoryId] ?: return null
        val title = "All $categoryId messages"
        val desc = "Set every rule of this category at once"
        val entry = bulkEntries.getOrPut("category:$categoryId") {
            ConfigEntry(categoryId, EntryKind.ENUM, title, desc, RuleAction.HIDE, RuleAction.entries.toList())
        }
        return Setting(entry, "category:$categoryId", title, Kind.DROPDOWN, desc, ids)
    }

    /** Valeur partagée par tous les réglages [ids], ou null s'ils divergent. */
    private fun commonValue(ids: List<String>): Enum<*>? {
        val entries = categoryConfig(currentCategory).entries
        return ids.mapNotNull { entries[it]?.getEnum() }.distinct().singleOrNull()
    }
    ...
    private fun buildItems() {
        ...
        val categoryRow = currentCategory?.let { categoryRow(it) }
        ...
        for ((title, ids) in sections) {
            val group = ids.mapNotNull { id ->
                val owner = MenuLayout.phraseOwner[id]
                if (owner != null && owner !in expanded) null else byId[id] ?: bulkRow(id)
            }
            if (group.isEmpty()) continue
            if (title.isNotEmpty()) items.add(Header(title))
            items.addAll(group)
    ...
    private fun renderRow(gfx: GuiGraphicsExtractor, row: Setting, top: Int, mx: Int, my: Int) {
        ...
        val label = when {
            row.id in MenuLayout.bulk -> "§l${if (row.id in expanded) "▾" else "▸"} ${row.title}"
            row.bulkIds != null -> "§l${row.title}"
            else -> row.title
        }
        gfx.text(font, label, mainX1() + 10, labelY, MenuTheme.TEXT)
        gfx.disableScissor()
        when (row.kind) {
            Kind.TOGGLE -> {
            ...
            Kind.DROPDOWN, Kind.TEXT, Kind.NUMBER -> {
                ...
                val value = when (row.kind) {
                    Kind.DROPDOWN ->
                        if (row.bulkIds != null) commonValue(row.bulkIds)?.name ?: "MIXED"
                        else row.entry.getEnum().name
    ...
    override fun mouseClicked(event: MouseButtonEvent, doubleClick: Boolean): Boolean {
        ...
        if (dd != null) {
            ...
            if (top != null) {
                ...
                for (c in dropdownConstants) {
                    if (inRect(mx, my, b[0], y, b[2], y + 15)) {
                        val bulk = dd.bulkIds
                        if (bulk == null) dd.entry.setEnum(c)
                        else categoryConfig(currentCategory).entries.let { e -> bulk.forEach { e[it]?.setEnum(c) } }
                        onValueChanged(); openDropdown = null; return true
                    }
                    y += 15
                }
            }
    ...
    private fun handleRowClick(row: Setting, top: Int, mx: Int, my: Int): Boolean = when (row.kind) {
        ...
        Kind.DROPDOWN -> {
            val b = widgetBoxAt(top)
            if (!inRect(mx, my, b[0], b[1], b[2], b[3])) {
                if (row.id !in MenuLayout.bulk) false
                else {
    ...
    companion object {
        ...
        private val FLAT_SECTIONS: Map<String?, LinkedHashMap<String, List<String>>> = mapOf(
            null to linkedMapOf(
                "" to listOf("masterEnabled", "groupRepeats", "smartCollapse", "updateNotifications", "maxMessages", "showTimestamps", "timestampColor", "highlightSelf", "selfColor", "mutedPlayers", "compactTheme", "compactThemeColor"),
                "CHAT TABS" to listOf("chatTabs", "tabFilterMode"),
            ),
        )

        // Plan des onglets/sections : construit depuis le registre (voir MenuLayout).
        private val VIEWS = MenuLayout.views
    }
}
