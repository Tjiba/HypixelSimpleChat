...
import com.simplechat.HoppityCompact
...
class ChatRulesTest {
    ...
    @Test fun `hide-useless off leaves message`() {
        val off = cfg.copy(groupActions = mapOf("transitions" to RuleAction.OFF))
        assertEquals(Verdict.Pass,
            ChatRules.evaluate("§aYou are playing on profile: §ePeach§b (Co-op)", off))
    }

    @Test fun `hide-useless group GREY dims instead of hiding`() {
        val grey = cfg.copy(groupActions = mapOf("transitions" to RuleAction.GREY))
